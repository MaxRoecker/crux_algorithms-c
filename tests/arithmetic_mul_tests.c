#include <tap.h>
#include "./../src/arithmetic/mul.h"




void CRUX__mul_char_tests (void) {
  ...
  if (CRUX__is_char_signed()) {
    ...
    const Char values[7][7] = {
      {0, 0, 0, 0, CHAR_MIN, 0, 0},
      {0, 0, CHAR_MAX, 0, CHAR_MIN + 1, 0, 0},
      {0, CHAR_MAX, 1, 0, -1, CHAR_MIN + 2, CHAR_MIN + 1},
      {0, 0, 0, 0, 0, 0, 0},
      {CHAR_MIN, CHAR_MIN + 1, -1, 0, 1, CHAR_MAX - 1, CHAR_MAX},
      {0, 0, CHAR_MIN + 2, 0, CHAR_MAX - 1, 0, 0},
      {0, 0, CHAR_MIN + 1, 0, CHAR_MAX, 0, 0}};
    for (Size i = as_size(0); i < as_size(7); i += as_size(1)) {
      for (Size j = as_size(0); j < as_size(7); j += as_size(1)) {
        ...
        CRUX__ResultChar result = CRUX__mul_char(a, b);
...
void CRUX__mul_size_tests (void) {
  ...
  const Size values[4][4] = {
    ...
    {SIZE_MIN, SIZE_MAX - 1, 0, 0},
    {SIZE_MIN, SIZE_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(4); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(4); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultSize result = CRUX__mul_size(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_iu08_tests (void) {
  ...
  const IU08 values[4][4] = {
    {IU08_MIN, IU08_MIN, IU08_MIN, IU08_MIN},
    {IU08_MIN, IU08_MIN + 1, IU08_MAX - 1, IU08_MAX},
    {IU08_MIN, IU08_MAX - 1, 0, 0},
    {IU08_MIN, IU08_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(4); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(4); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIU08 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_iu16_tests (void) {
  ...
  const IU16 values[4][4] = {
    {IU16_MIN, IU16_MIN, IU16_MIN, IU16_MIN},
    {IU16_MIN, IU16_MIN + 1, IU16_MAX - 1, IU16_MAX},
    {IU16_MIN, IU16_MAX - 1, 0, 0},
    {IU16_MIN, IU16_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(4); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(4); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIU16 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_iu32_tests (void) {
  ...
  const IU32 values[4][4] = {
    {IU32_MIN, IU32_MIN, IU32_MIN, IU32_MIN},
    {IU32_MIN, IU32_MIN + 1, IU32_MAX - 1, IU32_MAX},
    {IU32_MIN, IU32_MAX - 1, 0, 0},
    {IU32_MIN, IU32_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(4); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(4); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIU32 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_iu64_tests (void) {
  ...
  const IU64 values[4][4] = {
    {IU64_MIN, IU64_MIN, IU64_MIN, IU64_MIN},
    {IU64_MIN, IU64_MIN + 1, IU64_MAX - 1, IU64_MAX},
    {IU64_MIN, IU64_MAX - 1, 0, 0},
    {IU64_MIN, IU64_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(4); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(4); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIU64 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_is08_tests (void) {
  ...
  const IS08 values[7][7] = {
    {0, 0, 0, 0, IS08_MIN, 0, 0},
    {0, 0, IS08_MAX, 0, IS08_MIN + 1, 0, 0},
    {0, IS08_MAX, 1, 0, -1, IS08_MIN + 2, IS08_MIN + 1},
    {0, 0, 0, 0, 0, 0, 0},
    {IS08_MIN, IS08_MIN + 1, -1, 0, 1, IS08_MAX - 1, IS08_MAX},
    {0, 0, IS08_MIN + 2, 0, IS08_MAX - 1, 0, 0},
    {0, 0, IS08_MIN + 1, 0, IS08_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(7); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(7); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIS08 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_is16_tests (void) {
  ...
  const IS16 values[7][7] = {
    {0, 0, 0, 0, IS16_MIN, 0, 0},
    {0, 0, IS16_MAX, 0, IS16_MIN + 1, 0, 0},
    {0, IS16_MAX, 1, 0, -1, IS16_MIN + 2, IS16_MIN + 1},
    {0, 0, 0, 0, 0, 0, 0},
    {IS16_MIN, IS16_MIN + 1, -1, 0, 1, IS16_MAX - 1, IS16_MAX},
    {0, 0, IS16_MIN + 2, 0, IS16_MAX - 1, 0, 0},
    {0, 0, IS16_MIN + 1, 0, IS16_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(7); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(7); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIS16 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_is32_tests (void) {
  ...
  const IS32 values[7][7] = {
    {0, 0, 0, 0, IS32_MIN, 0, 0},
    {0, 0, IS32_MAX, 0, IS32_MIN + 1, 0, 0},
    {0, IS32_MAX, 1, 0, -1, IS32_MIN + 2, IS32_MIN + 1},
    {0, 0, 0, 0, 0, 0, 0},
    {IS32_MIN, IS32_MIN + 1, -1, 0, 1, IS32_MAX - 1, IS32_MAX},
    {0, 0, IS32_MIN + 2, 0, IS32_MAX - 1, 0, 0},
    {0, 0, IS32_MIN + 1, 0, IS32_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(7); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(7); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIS32 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
void CRUX__mul_is64_tests (void) {
  ...
  const IS64 values[7][7] = {
    {0, 0, 0, 0, IS64_MIN, 0, 0},
    {0, 0, IS64_MAX, 0, IS64_MIN + 1, 0, 0},
    {0, IS64_MAX, 1, 0, -1, IS64_MIN + 2, IS64_MIN + 1},
    {0, 0, 0, 0, 0, 0, 0},
    {IS64_MIN, IS64_MIN + 1, -1, 0, 1, IS64_MAX - 1, IS64_MAX},
    {0, 0, IS64_MIN + 2, 0, IS64_MAX - 1, 0, 0},
    {0, 0, IS64_MIN + 1, 0, IS64_MAX, 0, 0}};
  for (Size i = as_size(0); i < as_size(7); i += as_size(1)) {
    for (Size j = as_size(0); j < as_size(7); j += as_size(1)) {
      ...
      const Bool check = checks[i][j];
      CRUX__ResultIS64 result = CRUX__mul(a, b);
      const Bool actual_check = CRUX__trace_check(result.trace);
...
int main (int argc, char *argv[]) {
  plan(540);
  CRUX__mul_char_tests();
  CRUX__mul_size_tests();
  CRUX__mul_iu08_tests();
  CRUX__mul_iu16_tests();
  CRUX__mul_iu32_tests();
  CRUX__mul_iu64_tests();
  CRUX__mul_is08_tests();
  CRUX__mul_is16_tests();
  CRUX__mul_is32_tests();
  CRUX__mul_is64_tests();
  done_testing();
  return EXIT_SUCCESS;
}
